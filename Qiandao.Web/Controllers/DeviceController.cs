...
namespace Qiandao.Web.Controllers
{
    ...
    public class DeviceController : ControllerBase
    {
        ...
        [HttpGet("setSpecificPersonToDevice")]
        public IActionResult setSpecificPersonToDevice(string deviceSn , string userIds)
        {
            if (personService == null)
            {
                return BadRequest("personService service not initialized.");
            }
            personService.setSpecificUserToDevice2(deviceSn,userIds);
                return Ok(new { code = 0, msg = "success" });
        ...
        public IActionResult records(int page, int limit, string? deviceSn)
        {
            ...
            if (model.Code == 200)
            {
                return Ok(new { code = 0, msg = "Success", data = model.Data, count = modelx.Data.Count });
            }
            else
            {
        ...
        public IActionResult getAllLog(string? deviceSn)
        {
            ...
            if (model.Code == 200)
            {
                return Ok(new { code = 0, msg = "Success", data = model.Data, count = model.Data.Count });
            }
            else
            {
                return BadRequest(new { code = 1, msg = "失败" });
